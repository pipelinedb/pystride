...
def check_request(request):
    ...
    assert request.headers['User-Agent'] == ('pystride (version: %s)'
                                             % __version__)
...
def rsps():
    def subscribe(request):
        ...
        events = [{'$id': i, '$timestamp': str(datetime.utcnow())}
                  for i in xrange(100)]
    ...
    rsps.add_callback(
        responses.GET, 'https://api.stride.io/v1/collect/stream/subscribe',
        callback=subscribe,
        content_type='application/json'
    )
    rsps.add_callback(
        responses.POST, 'https://api.stride.io/v1/process/p1',
        callback=post,
        content_type='application/json'
    )
    return rsps
